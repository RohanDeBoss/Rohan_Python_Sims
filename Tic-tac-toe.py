...
class StoppableAIThread(threading.Thread):
    def __init__(self, *args, **kwargs):
        super(StoppableAIThread, self).__init__(*args, **kwargs)
    ...
    def run(self):
        if self._target == find_best_move_iterative_deepening:
            new_args = list(self._args)
            new_args.append(self._stop_event)
            self._args = tuple(new_args)
        super(StoppableAIThread, self).run()
...
def _generate_masks_for_k(board_size, k_to_win):
    ...
    for r in range(board_size):
        for c in range(board_size - k_to_win + 1):
            ...
            masks.append(mask)
    # Vertical
    for c_idx_mask in range(board_size):
        for r in range(board_size - k_to_win + 1):
            mask = 0
            for i in range(k_to_win):
                mask |= (1 << cell_to_bit_index(r + i, c_idx_mask, board_size))
            masks.append(mask)
    # Diagonal \
    for r in range(board_size - k_to_win + 1):
        for c in range(board_size - k_to_win + 1):
...
def get_winning_line(board, player, k_to_win):
    ...
    # Vertical
    for c_idx_wl in range(n):
        for r_idx in range(n - k_to_win + 1):
            if all(board[r_idx+i][c_idx_wl] == player for i in range(k_to_win)): return [(r_idx+i, c_idx_wl) for i in range(k_to_win)]
...
def _rotate_bitboard_90_cw(bb, board_size):
    ...
    while temp_bb > 0:
        ...
        r, c_rot = bit_index_to_cell(idx, board_size)
        new_idx = cell_to_bit_index(c_rot, board_size - 1 - r, board_size)
        new_bb |= (1 << new_idx)
        temp_bb ^= lsb_mask
    return new_bb
...
def _reflect_bitboard_horizontal(bb, board_size):
    ...
    while temp_bb > 0:
        lsb_mask = temp_bb & -temp_bb
        idx = lsb_mask.bit_length() - 1
        r, c_refl = bit_index_to_cell(idx, board_size)
        new_idx = cell_to_bit_index(r, board_size - 1 - c_refl, board_size)
        new_bb |= (1 << new_idx)
        temp_bb ^= lsb_mask
    return new_bb
...
def get_available_moves_bb(player_x_bb, player_o_bb, board_size, center_sort=True):
    ...
    temp_available_bb = available_bb
...
def _get_immediate_winning_moves_bb(player_x_bb, player_o_bb, current_player_id, k_to_win, board_size):
    ...
    for mask in win_masks:
        ...
        if needed_for_win_mask != 0 and (needed_for_win_mask & (needed_for_win_mask - 1)) == 0:
            move_bit = needed_for_win_mask

            if (move_bit & occupied_bb) == 0:
                move_idx = move_bit.bit_length() - 1
...
def can_force_win_on_next_player_turn_bb(player_x_bb, player_o_bb, current_player_id, k_to_win, board_size):
    ...
    for m1_idx in original_player_moves:
        ...
        for mopp_idx in opponent_moves_after_m1:
            ...
            if check_win_bb(opponent_bb_after_mopp, board_size, k_to_win):
                m1_is_forcing = False; break
...
def _minimax_recursive_logic_bb(canonical_x_bb, canonical_o_bb, depth, is_maximizing_for_x, k_to_win, board_size, alpha, beta, current_max_search_depth_for_this_call, current_zobrist_hash, stop_event=None):
    ...
    current_player_id_at_node = PLAYER_IDS[PLAYER_HUMAN] if is_maximizing_for_x else PLAYER_IDS[PLAYER_AI]
    opponent_player_id_at_node = 1 - current_player_id_at_node

    tt_entry_for_lookup = transposition_table.get(current_zobrist_hash) # Use .get() for cleaner check

    if tt_entry_for_lookup and tt_entry_for_lookup['depth_searched'] >= remaining_depth_tt:
        tt_hits += 1
        if tt_entry_for_lookup['flag'] == TT_EXACT: return (tt_entry_for_lookup['score'], nodes_here, max_depth_here)
        elif tt_entry_for_lookup['flag'] == TT_LOWER_BOUND: alpha = max(alpha, tt_entry_for_lookup['score'])
        elif tt_entry_for_lookup['flag'] == TT_UPPER_BOUND: beta = min(beta, tt_entry_for_lookup['score'])
        if alpha >= beta: return (tt_entry_for_lookup['score'], nodes_here, max_depth_here)
    ...
    if score_terminal is not None:
        transposition_table[current_zobrist_hash] = {'score': score_terminal, 'depth_searched': 999, 'flag': TT_EXACT, 'best_move_idx': None}
    ...
    if (depth + 3) <= current_max_search_depth_for_this_call:
        ...
        if can_force_win_on_next_player_turn_bb(canonical_x_bb, canonical_o_bb, current_player_id_at_node, k_to_win, board_size):
            score_forced = (1000 - (depth + 2)) if is_maximizing_for_x else (-1000 + (depth + 2))
            is_forced = True
        if is_forced:
            transposition_table[current_zobrist_hash] = {'score': score_forced, 'depth_searched': 999, 'flag': TT_EXACT, 'best_move_idx': None}
    ...
    # 1. TT Hash Move
    tt_hash_move = None
    if tt_entry_for_lookup and tt_entry_for_lookup.get('best_move_idx') is not None:
        tt_hash_move = tt_entry_for_lookup['best_move_idx']
        ordered_moves.append(tt_hash_move)

    # 2. Immediate Winning Moves for Current Player
    immediate_wins = _get_immediate_winning_moves_bb(canonical_x_bb, canonical_o_bb, current_player_id_at_node, k_to_win, board_size)
    for move_idx in immediate_wins:
        if move_idx not in ordered_moves:
            ordered_moves.append(move_idx)

    # 3. Moves that Block Opponent's Immediate Win
    opponent_wins = _get_immediate_winning_moves_bb(canonical_x_bb, canonical_o_bb, opponent_player_id_at_node, k_to_win, board_size)
    for move_idx in opponent_wins:
        if move_idx not in ordered_moves:
            ordered_moves.append(move_idx)

    # 4. Remaining moves (center-sorted)
    all_available_moves = get_available_moves_bb(canonical_x_bb, canonical_o_bb, board_size, center_sort=True)
    for move_idx in all_available_moves:
        if move_idx not in ordered_moves:
            ordered_moves.append(move_idx)
    
    if not ordered_moves: return (0, nodes_here, max_depth_here)
    # --- End Enhanced Move Ordering ---
    ...
    if is_maximizing_for_x:
        max_eval = -math.inf
        for i, move_idx in enumerate(ordered_moves):
            if stop_event and stop_event.is_set(): return (max_eval if max_eval != -math.inf else 0, nodes_here, max_depth_here)
            ...
            if i == 0:
                current_eval_score, child_nodes_iter, child_max_depth_iter = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, False, k_to_win, board_size, alpha, beta, current_max_search_depth_for_this_call, child_hash, stop_event)
            else:
                null_window_score, child_nodes_null, child_max_depth_null = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, False, k_to_win, board_size, alpha, alpha + 1, current_max_search_depth_for_this_call, child_hash, stop_event)
                ...
                if null_window_score > alpha and null_window_score < beta:
                    if stop_event and stop_event.is_set(): return (max_eval if max_eval != -math.inf else 0, nodes_here, max_depth_here)
                    current_eval_score, child_nodes_re, child_max_depth_re = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, False, k_to_win, board_size, alpha, beta, current_max_search_depth_for_this_call, child_hash, stop_event)
        ...
        min_eval = math.inf
        for i, move_idx in enumerate(ordered_moves):
            if stop_event and stop_event.is_set(): return (min_eval if min_eval != math.inf else 0, nodes_here, max_depth_here)
            ...
            current_eval_score = 0; child_nodes_iter = 0; child_max_depth_iter = 0
            if i == 0:
                current_eval_score, child_nodes_iter, child_max_depth_iter = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, True, k_to_win, board_size, alpha, beta, current_max_search_depth_for_this_call, child_hash, stop_event)
            else:
                null_window_score, child_nodes_null, child_max_depth_null = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, True, k_to_win, board_size, beta - 1, beta, current_max_search_depth_for_this_call, child_hash, stop_event)
                child_nodes_iter += child_nodes_null; child_max_depth_iter = max(child_max_depth_iter, child_max_depth_null)

                if null_window_score < beta and null_window_score > alpha:
                    if stop_event and stop_event.is_set(): return (min_eval if min_eval != math.inf else 0, nodes_here, max_depth_here)
                    current_eval_score, child_nodes_re, child_max_depth_re = _minimax_recursive_logic_bb(child_canonical_x_bb, child_canonical_o_bb, depth + 1, True, k_to_win, board_size, alpha, beta, current_max_search_depth_for_this_call, child_hash, stop_event)
                    child_nodes_iter += child_nodes_re
                    child_max_depth_iter = max(child_max_depth_iter, child_max_depth_re)
                else:
                    current_eval_score = null_window_score
    ...
    if not (stop_event and stop_event.is_set()):
        ...
        # Use tt_entry_for_lookup (which could be None if no prior entry existed)
        # to check existing entry before overwriting
        existing_tt_entry_for_store_check = transposition_table.get(current_zobrist_hash) # Re-fetch just before store, in case of concurrent access (though not an issue with single AI thread)
        if (existing_tt_entry_for_store_check is None or
                remaining_depth_tt > existing_tt_entry_for_store_check['depth_searched'] or
                (remaining_depth_tt == existing_tt_entry_for_store_check['depth_searched'] and flag == TT_EXACT and existing_tt_entry_for_store_check['flag'] != TT_EXACT)):
...
def minimax_iterative_bb(player_x_bb, player_o_bb, is_turn_of_maximizer_x, k_to_win, board_size, search_ply_limit_from_here, stop_event=None):
    ...
    return _minimax_recursive_logic_bb(canonical_x, canonical_o, 0, is_turn_of_maximizer_x, k_to_win, board_size, -math.inf, math.inf, search_ply_limit_from_here, initial_hash, stop_event)

def find_best_move_iterative_deepening(initial_board_list, k_to_win_config, progress_q, result_q, player_token_for_move, stop_event):
    ...
    if stop_event.is_set(): result_q.put({'best_move_data': (None, 0), 'top_moves_list': [], 'total_nodes': 0, 'achieved_relative_depth': 0, 'stopped': True}); return
    ...
    if immediate_wins_indices:
        best_move_idx = immediate_wins_indices[0]
    ...
    if opponent_immediate_wins_indices and len(opponent_immediate_wins_indices) == 1:
        ...
        if must_block_idx in root_available_moves_indices:
            root_available_moves_indices = [must_block_idx]
    ...
    for current_iddfs_ply_limit in range(1, max_remaining_plies + 1):
        if stop_event.is_set(): result_q.put({'best_move_data': (bit_index_to_cell(overall_best_move_idx, current_board_size) if overall_best_move_idx is not None else None, overall_best_score), 'top_moves_list': last_iter_full_eval_details_rc[:5], 'total_nodes': accumulated_total_nodes, 'achieved_relative_depth': final_iddfs_ply_limit_for_best_move, 'stopped': True}); return
        ...
        for i, root_move_idx in enumerate(root_available_moves_indices):
            ...
            eval_score, nodes_in_branch, actual_depth_this_branch_relative = minimax_iterative_bb(child_x_bb, child_o_bb, next_player_is_maximizer_x, k_to_win_config, current_board_size, minimax_search_ply_limit_for_children, stop_event)
            if stop_event.is_set(): result_q.put({'best_move_data': (bit_index_to_cell(overall_best_move_idx, current_board_size) if overall_best_move_idx is not None else None, overall_best_score), 'top_moves_list': last_iter_full_eval_details_rc[:5], 'total_nodes': accumulated_total_nodes, 'achieved_relative_depth': final_iddfs_ply_limit_for_best_move, 'stopped': True}); return
        ...
        if current_iter_best_move_idx_candidate is not None:
            ...
            if (is_maximizing_search and current_iter_best_score_candidate >= (1000 - current_iddfs_ply_limit)) or \
               (not is_maximizing_search and current_iter_best_score_candidate <= (-1000 + current_iddfs_ply_limit)):
                 if abs(current_iter_best_score_candidate) >= (1000 - max_remaining_plies) :
                    break

    if stop_event.is_set():
        final_move_rc = bit_index_to_cell(overall_best_move_idx, current_board_size) if overall_best_move_idx is not None else None
        final_top_moves = last_iter_full_eval_details_rc[:5] if last_iter_full_eval_details_rc else []
        result_q.put({
            'best_move_data': (final_move_rc, overall_best_score),
            'top_moves_list': final_top_moves,
    ...
    if overall_best_move_idx is None and root_available_moves_indices:
        overall_best_move_idx = root_available_moves_indices[0]
    ...
    if last_iter_full_eval_details_rc:
        ...
        final_top_moves_list_rc = [{'move': final_best_move_rc, 'score': overall_best_score, 'nodes': accumulated_total_nodes if accumulated_total_nodes >0 else 1, 'depth': final_iddfs_ply_limit_for_best_move, 'actual_eval_depth': final_iddfs_ply_limit_for_best_move}]
...
class TicTacToeGUI:
    ...
    def __init__(self, root_window):
        ...
        self._current_calculation_for_player = None
    ...
    def on_silent_close(self, event=None):
        if self.calculation_manager_thread and self.calculation_manager_thread.is_alive():
            self.calculation_manager_thread.stop()
            self.calculation_manager_thread.join(timeout=0.5)
        self.root.destroy()
    ...
    def start_new_game(self, human_starts=True):
        ...
        try:
            ...
            WINNING_MASKS_CACHE.clear(); POTENTIAL_LINE_MASKS_CACHE.clear()
            get_win_masks(self.board_size, self.k_to_win); get_potential_line_masks(self.board_size, self.k_to_win)
            compute_zobrist_hash_from_canonical_bbs.cache_clear(); get_canonical_bitboards.cache_clear()
            check_win_bb.cache_clear(); is_board_full_bb.cache_clear(); _get_immediate_winning_moves_bb.cache_clear()
            can_force_win_on_next_player_turn_bb.cache_clear(); is_unwinnable_for_either_bb.cache_clear()
        ...
        if not human_starts:
            self._current_calculation_for_player = PLAYER_AI
            self.root.after(100, lambda: self.trigger_ai_or_hint_calculation(PLAYER_AI))

    def update_hint_button_state(self):
        is_calc_running = bool(self.calculation_manager_thread and self.calculation_manager_thread.is_alive() and self._current_calculation_for_player is not None)
    ...
    def get_human_hint(self):
        ...
        if self.calculation_manager_thread and self.calculation_manager_thread.is_alive() and self._current_calculation_for_player is not None:
             messagebox.showinfo("Busy", "AI calculation is currently in progress.")
             return
    ...
    def trigger_ai_or_hint_calculation(self, player_to_calculate_for):
        ...
        current_thread = StoppableAIThread(target=find_best_move_iterative_deepening,
                                           args=(self.game_board, self.k_to_win,
                                                 self.progress_queue, self.result_queue,
                                                 player_to_calculate_for),
                                           daemon=True)
        self.calculation_manager_thread = current_thread
    ...
    def create_board_ui_buttons(self, attempt=1):
        ...
        for r in range(self.board_size):
            row_buttons = []
            for c_idx_btn in range(self.board_size):
                 button = tk.Button(self.board_frame, text=EMPTY_CELL, font=board_button_font,
                                   relief="flat", borderwidth=1, bg=COLOR_FRAME_BG,
                                   activebackground="#DDDDDD", fg=COLOR_TEXT_PRIMARY,
                                   command=lambda r_idx=r, c_idx_cmd=c_idx_btn: self.handle_cell_click(r_idx, c_idx_cmd))
                 button.grid(row=r, column=c_idx_btn, sticky="nsew", padx=1, pady=1)
    ...
    def handle_cell_click(self, r, c):
        ...
        if self.calculation_manager_thread and self.calculation_manager_thread.is_alive() and self._current_calculation_for_player is not None:
             messagebox.showinfo("Wait", "AI calculation is currently in progress.")
             return
    ...
    def check_ai_progress(self):
        ...
        active_thread = self.calculation_manager_thread
        if not active_thread or self._current_calculation_for_player is None or active_thread.stopped():
            if self.root.cget('cursor') == "watch": self.root.config(cursor="")
            if self.progress_bar.master.winfo_ismapped(): self.progress_bar.master.grid_remove()
            if self.detailed_progress_label.winfo_ismapped(): self.detailed_progress_label.grid_remove()
            if not self.game_over: self.enable_board_buttons()
            self.update_hint_button_state()
            if (active_thread and active_thread.stopped()) or self._current_calculation_for_player is None:
                 while not self.progress_queue.empty():
                    try: self.progress_queue.get_nowait()
                    except thread_queue.Empty: break
                 while not self.result_queue.empty():
                 ...
                 if self.calculation_manager_thread == active_thread :
                    self.calculation_manager_thread = None
            return

        is_calculating = active_thread.is_alive()

        if self._current_calculation_for_player is not None:
            if is_calculating:
            ...
            for _ in range(max_messages_to_process):
                if not active_thread.is_alive() or active_thread.stopped(): break
                try:
            ...
            if is_calculating and not active_thread.stopped():
        ...
        if is_calculating and not active_thread.stopped():
            ...
            if self._current_calculation_for_player is not None and not active_thread.stopped():
                ...
                while not self.progress_queue.empty():
                    try:
                        ...
                        if 'current_nodes' in final_prog_update: latest_nodes = final_prog_update['current_nodes']
                        if 'current_best_score' in final_prog_update: latest_best_score_so_far = final_prog_update['current_best_score']
                    except thread_queue.Empty: break
                if latest_nodes is not None: self.nodes_explored_var.set(f"{latest_nodes:,}"); self.nps_var.set(f"{(latest_nodes / final_runtime):,.0f}" if final_runtime > 0.01 else ("High" if latest_nodes > 0 else "0"))
                else: self.nodes_explored_var.set("0"); self.nps_var.set("N/A")
                if latest_best_score_so_far is not None: self.ai_eval_var.set(f"{latest_best_score_so_far:.0f}" if isinstance(latest_best_score_so_far, (int, float)) and latest_best_score_so_far not in (math.inf, -math.inf) else str(latest_best_score_so_far))
            ...
            if self._current_calculation_for_player is not None and not (active_thread and active_thread.stopped()):
                self.progress_percent_var.set("100%")
                self.detailed_progress_var.set("Search complete.")
            else:
                self.progress_percent_var.set("Cancelled" if (active_thread and active_thread.stopped()) else "Done")
                self.detailed_progress_var.set("Search cancelled." if (active_thread and active_thread.stopped()) else "Search finished.")
                self._current_calculation_for_player = None
    ...
    def handle_calculation_result(self):
        ...
        current_calc_player_at_start = self._current_calculation_for_player
        if current_calc_player_at_start is None:
            while not self.result_queue.empty():
                try: self.result_queue.get_nowait()
                except thread_queue.Empty: break
            if not self.game_over : self.enable_board_buttons()
        ...
        try:
            time_taken_calc = float(self.time_taken_var.get())
        except ValueError:
            time_taken_calc = time.monotonic() - self.ai_start_time
            if time_taken_calc <= 0: time_taken_calc = 0.001
            self.time_taken_var.set(f"{time_taken_calc:.3f}")

        was_hint = (current_calc_player_at_start == PLAYER_HUMAN)

        try:
            res = self.result_queue.get_nowait()
            if res.get('stopped', False) and self._current_calculation_for_player is not None :
                if not self.game_over : self.enable_board_buttons()
                self.update_hint_button_state()
                self._current_calculation_for_player = None
                return
            ...
            if not self.game_over:
                ...
                if self._current_calculation_for_player and ("calculating" in current_status.lower() or "thinking" in current_status.lower()):
                     self._set_status_normal("Search interrupted or no result.")
            if not self.game_over : self.enable_board_buttons()
            self.update_hint_button_state()
            self._current_calculation_for_player = None
            return
        ...
        if was_hint:
            if move:
                self._set_status_as_hint(f"Suggested for X: {move_alg} (Score: {score:.0f})")
                if not self.game_over and self.game_board[move[0]][move[1]] == EMPTY_CELL and self.buttons[move[0]][move[1]]['state']!=tk.DISABLED:
                    self.buttons[move[0]][move[1]].config(bg=COLOR_HINT_SUGGEST_BG, relief="raised")
            else:
                self._set_status_as_hint("Hint: No moves or game ended.")
        else:
            ...
            if move:
                ...
                if is_board_full_bb(current_x_bb, current_o_bb, self.board_size):
                    self._set_status_normal("It's a Draw!")
                else:
                     self._set_status_normal("AI error/no moves. Game Over?")
                self.game_over = True
    ...
    def update_board_button_states(self):
        ...
        is_ai_thinking_currently = bool(self.calculation_manager_thread and self.calculation_manager_thread.is_alive() and self._current_calculation_for_player is not None)

        for r, row_btns in enumerate(self.buttons):
            for c_upd, button in enumerate(row_btns):
                if not button: continue
                current_cell_text = self.game_board[r][c_upd]
                is_hinted = button.cget('bg') == COLOR_HINT_SUGGEST_BG

                if self.game_over or is_ai_thinking_currently:
                    if current_cell_text == EMPTY_CELL:
                        button.config(text=current_cell_text, state=tk.DISABLED,
                    ...
                    if current_cell_text == EMPTY_CELL:
                        button.config(text=current_cell_text, state=tk.NORMAL,
                                      fg=COLOR_TEXT_PRIMARY,
                                      bg=COLOR_HINT_SUGGEST_BG if is_hinted else COLOR_FRAME_BG,
    ...
    def highlight_winning_line(self, winning_cells):
        if not self.buttons: return
        for r,c_hl in winning_cells:
             if r < len(self.buttons) and self.buttons[r] and c_hl < len(self.buttons[r]) and self.buttons[r][c_hl]: self.buttons[r][c_hl].config(background=COLOR_WIN_HIGHLIGHT, relief=tk.GROOVE)
